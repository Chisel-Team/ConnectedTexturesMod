...
public class TextureTypeRegistry {
    ...
    public static void preInit(FMLPreInitializationEvent event) {
        Set<ASMData> annots = event.getAsmData().getAll(TextureType.class.getName());
        for (ASMData data : annots) {
            ...
            try {
                register(name, ((Class<? extends ITextureType>) Class.forName(data.getClassName())).newInstance());
            } catch (InstantiationException e) {
                // This might be a field, let's try that
                try {
                    Class<?> c = Class.forName(data.getClassName());
                    register(name, (ITextureType) c.getDeclaredField(data.getObjectName()).get(null));
